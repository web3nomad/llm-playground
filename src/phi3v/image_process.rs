...
impl Phi3VImageProcessor {
    ...
    pub fn preprocess(&self, image: &DynamicImage) -> Result<BatchFeature> {
        let rgb_image = if self.do_convert_rgb {
            image.to_rgb8()
        } else {
            image.to_rgb8()
        };
    ...
    fn normalize_image(&self, image: &DynamicImage) -> Array4<f32> {
        ...
        for (x, y, pixel) in image.pixels() {
            for c in 0..3 {
                normalized[[0, c, y as usize, x as usize]] =
                    (pixel[c] as f32 / 255.0 - self.image_mean[c]) / self.image_std[c];
            }
        }
            ...
            for w in (0..width).step_by(336) {
                let patch = image
                    .slice(s![
                        ..,
                        ..,
                        h..std::cmp::min(h + 336, height),
                        w..std::cmp::min(w + 336, width)
                    ])
                    .to_owned();
                patches.push(patch);
            }
    ...
    fn pad_to_max_num_crops_tensor(
        &self,
        patches: &[Array4<f32>],
        max_crops: usize,
    ) -> Array4<f32> {
