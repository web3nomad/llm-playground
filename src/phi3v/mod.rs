...
pub async fn generate_text(
    ...
    let (mut inputs_embeds, mut attention_mask) = {
        ...
        if image.is_some() {
            ...
            let image_token_position = input_ids
                .iter()
                .position(|&id| id == USER_TOKEN_ID)
                .unwrap_or(0);
            (inputs_embeds, attention_mask) = merge_text_and_image_embeddings(
                &inputs_embeds,
                &attention_mask,
                &visual_features,
    ...
    for _ in 0..MAX_LENGTH {
        ...
        let logits: ArrayView<f32, _> = model_outputs["logits"]
...
pub async fn run() -> Result<()> {
    ...
    let data_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("models/phi-3-vision");
